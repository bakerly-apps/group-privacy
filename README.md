# GROUP — privacy policy

Published at <https://bakerly-apps.github.io/group-privacy/> and referenced from
the Google Play listing for `com.bakerly.group`.

**Do not edit `index.html` here.** It is generated. The source of truth is
`docs/PRIVACY_POLICY.md` in the game project; regenerate with `npm run privacy`
after editing `store/privacy.config.json`, then copy the output over `index.html`
and push.

Re-publish on the same day any of these ship, because the policy states they are
inactive: advertising, in-app purchases, analytics, crash reporting, the daily
leaderboard, community levels.
